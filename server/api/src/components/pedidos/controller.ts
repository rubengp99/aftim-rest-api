...
export async function getStats(): Promise<any> {
	try {
		let statusNuevo = await consult.countOther("rest_estatus", model, "1");
		let statusFacturado = await consult.countOther("rest_estatus", model, "2");
		let statusPorFacturar = await consult.countOther("rest_estatus", model, "3");
		let data1 = await consult.getPersonalized("SELECT COUNT(*), rest_pedidos.adm_empresa_id FROM rest_pedidos  GROUP BY rest_pedidos.adm_empresa_id");
