    ...
    try {
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            if (!fields || fields.includes("presentaciones")) {
                let pres: any[] = await consult.getOtherByMe(
                    model,
                    id as string,
                    submodel,
                    {}
            ...
            if (!fields || fields.includes("existencias")) {
                let movDep: any[] = await consult.getOtherByMe(
                    model,
                    id as string,
                    "adm_movimiento_deposito",
                    { fields: "id,adm_depositos_id,existencia" }
                );
                data[i].existencias = movDep;
            }
            if (fields && fields.includes("grupo")) {
                data[i].grupo = await consult.getOne(
                    "adm_grupos",
                    id as string,
                    { fields: "*" }
                );
            ...
            if (fields && fields.includes("subgrupo")) {
                data[i].subgrupo = await consult.getOne(
                    "adm_subgrupos",
                    id as string,
                    { fields: "*" }
                );
...
export const getOne = async (id: string | number, query: any): Promise<any> => {
    try {
        ...
        if (!fields || fields.includes(submodel)) {
            let pres = (await consult.getOtherByMe(
                model,
                id as string,
                submodel,
                {}
...
export async function isSold(id: string | number): Promise<any> {
    try {
        ...
        if (count <= 0)
            return { response: { isSold: false }, code: respuestas.Ok.code };
 ...
 * @param query modifier of the consult
 */
export const getDepositsByConcept = async (
    id: string | number,
    query: any
): Promise<any> => {
    try {
        ...
        let data: any = await consult.getOtherByMe(
            model,
            id,
            "adm_movimiento_deposito",
            { fields: "id,adm_depositos_id,existencia" }
        ...
        let link = links.pages(
            data,
            `conceptos/${id}/depositos`,
            count,
            totalCount,
            limit
        );
 ...
 * @param query modifier of the consult
 */
export const getDepositMovementsByConcept = async (
    id: string | number,
    query: any
): Promise<any> => {
    try {
 ...
 * @param query modifier of the consult
 */
export const getPhotosByConcept = async (
    id: string | number,
    query: any
): Promise<any> => {
    try {
 ...
 * @param query modifier of the consult
 */
export const getPresentationsByConcept = async (
    id: string | number,
    query: any
): Promise<any> => {
    try {
...
export async function getMostSold(query: any): Promise<any> {
    ...
    try {
        ...
        ${
            query["after-fecha_at"]
                ? `AND adm_det_facturas.fecha_at >= '${query["after-fecha_at"]}'`
                : ""
        }
        ${
            query["before-fecha_at"]
                ? ` AND adm_det_facturas.fecha_at <= '${query["before-fecha_at"]}'`
                : ""
        }
        GROUP BY adm_conceptos_id ORDER BY vendidos ${
            order ? order : "desc"
...
export async function getMostReturned(query: any): Promise<any> {
    ...
    try {
        ...
        GROUP BY adm_conceptos_id ORDER BY devueltos ${
            order ? order : "desc"
        } LIMIT ${limit ? limit : 10}`;
        console.log(sql);
        let data: any[] = await consult.getPersonalized(sql);
...
export async function devolutionsByConcept(
    params: any,
    query: any
): Promise<any> {
    try {
...
export const update = async (
    ...
    try {
        ...
        let { affectedRows } = (await consult.update(
            model,
            id,
            newGrupo
        )) as any;
        ...
        let response = Object.assign(
            { message: respuestas.Update.message, affectedRows },
            { link: link }
        );
